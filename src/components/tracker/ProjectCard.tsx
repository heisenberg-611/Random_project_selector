...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <div
      ref={cardRef}
      className={`salam-bevel relative p-5 sm:p-6 space-y-4 h-fit ${
        isExpanded ? 'z-30 border-[#C9A76C]/60 shadow-xl shadow-[#C9A76C]/5' : 'z-10'
    ...
    >
      ...
      <div className="pt-3 border-t border-white/[0.08] flex flex-col xs:flex-row items-stretch xs:items-center justify-between gap-2.5">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 text-[11px] font-mono px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer ${
            isExpanded
              ? 'bg-[#C9A76C]/20 text-[#E4CCA1] border-[#C9A76C]/50 shadow-sm'
              : 'bg-[#121216] text-[#9CA3AF] hover:text-[#EDE8E8] border-white/[0.08] hover:border-white/20'
          }`}
        >
          <FileCode className="w-3 h-3 text-[#C9A76C]" />
          <span>{isExpanded ? 'CLOSE_DOCS' : 'SPEC_DOCS'}</span>
          {isExpanded ? (
            <ChevronUp className="w-3.5 h-3.5 text-[#C9A76C]" />
          ) : (
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 bg-[#121216]/98 backdrop-blur-2xl border border-[#C9A76C]/40 rounded-xl p-4 sm:p-5 shadow-2xl shadow-black/95 font-mono text-xs text-[#EDE8E8] max-h-[440px] overflow-y-auto scrollbar-thin"
          >
            {/* Popover Header Bar */}
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-white/[0.08]">
              <div className="flex items-center gap-2 text-[11px] font-mono font-bold text-[#C9A76C]">
                <Terminal className="w-3.5 h-3.5" />
                <span>SPEC_MANIFEST // {project.title.slice(0, 30)}{project.title.length > 30 ? '...' : ''}</span>
              </div>
